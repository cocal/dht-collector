...
const RECENT_RESOURCE_CACHE_LIMIT = 50_000
...
class PostgresCatalog {
  ...
  async listRecentResourceObservations(cutoff) {
    const { rows } = await this.pool.query(`
      ...
      LIMIT $2
    `, [cutoff, RECENT_RESOURCE_CACHE_LIMIT])
  ...
  async #writeEventBatch(batch) {
    ...
    for (const event of manifests) {
      const inserted = await this.upsertManifest(event.manifest)
      if (inserted) {
        await this.pool.query(`
          INSERT INTO minute_metric (bucket, indexed)
          VALUES (date_trunc('minute', $1::timestamptz), 1)
          ON CONFLICT (bucket) DO UPDATE SET
            indexed = minute_metric.indexed + excluded.indexed
        `, [event.occurred_at])
      }
    }
  }
  ...
  async upsertManifest(manifest) {
    ...
    try {
      ...
      const result = await client.query(`
        ...
        SELECT coalesce(bool_or(upsert.inserted), false) AS inserted
        FROM upsert CROSS JOIN counter
