...
record Config(String mode, String address, int port, int dhtNodes, int maxConcurrent, int maxResources,
  ...
  static Config from(String[] args) {
    ...
    return new Config(mode, address, port, nodes, concurrent, max, url, user, password, pool, storage, httpHost, httpPort, metadataConcurrent, metadataTimeout, staticPath);
  }
}
